    ...
    if ip_file:
        with open(ip_file, "r") as csv_file:
            ...
            for row in csv_reader:
                ...
                # strip sentence if it exceeds the max length
                if len(lst_tokens) > max_len:
                    lst_tokens = lst_tokens[:max_len]

                # pad the sentence if length is less than max length
...
# reading in external features - labMT, AFINN, etc. 
    ...
    with open(op_file, 'w') as of:
        ...
        for train_index, test_index in skf.split(x_data, y_data):
            ...
            for index, cnn_feature in enumerate(train_cnn_feat):
                ...
                # comment when running without external features
